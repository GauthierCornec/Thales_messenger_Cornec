...
const Messenger = () => {
      ...
      axios
        ...
        .then((r) => {
          ...
          if (selectedConversationId) {
            const selectedConversation = r.data.conversations.find(
  ...
  const updateConversations = (newConversation) => {
    setUserData((prevUserData) => {
      ...
      if (existingConversationIndex !== -1) {
        ...
        if (
          selectedContactInfo &&
          selectedContactInfo.conversations[0].id === newConversation.id
        ) {
      ...
      updatedUserData.conversations = conversations.sort(
  ...
  const sendMessage = () => {
    AsyncStorage.getItem("token").then((value) => {
      ...
      axios
        .post(`http://${process.env.REACT_APP_API_URL}/messages`, payload, {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + value,
          },
        ...
        .then((response) => {
          ...
          setSelectedContactMessages((prevMessages) => [
            ...prevMessages,
            newMessage,
          ]);
          setMessageContent("");
  ...
  const setSelectedContact = (contact) => {
    ...
    const selectedConversation = userData.conversations.find(
  ...
  return (
    <div className="flex">
      <div
        className={`flex flex-col bg-slate-100 max-w-sm ${
      ...
      >
        ...
        <div className="relative mb-4">
          ...
          <button
            type="submit"
            className="text-black absolute right-2.5 bottom-2.5 bg-gray-200 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2"
          >
            Search
          </button>
        </div>
          ...
          <div className="flex- overflow-auto rounded-lg place-items-center h-full">
            {userData.contacts
              ...
              .map((contact) => {
                ...
                return (
                  <div
                    className="flex flex-row shadow-inner p-2"
                    key={contact.id}
                  ...
                  >
                    ...
                    <div className="ml-8 flex flex-col">
                      ...
                      <div className="truncate" style={{ width: "200px" }}>
                        {contactConversations.reverse().map((conversation) => (
                          <p
                            key={conversation.id}
                            style={{
                              overflow: "hidden",
      ...
      <div
        className={`flex flex-col w-full relative mx-4 bg-slate-100 max-h-screen ${
          selectedContactInfo ? "block" : "hidden"
        }`}
      >
